(function (root, factory) {
  ...
  const sessionCategories = new Set(['running','swimming','cycling','strength','hyrox','metcon','test','recovery','other']);
  ...
  function validateSession(session) {
    ...
    if (session.outcome !== null && session.outcome !== undefined) {
      ...
      if(owns(session.outcome,'strengthPerformance')){
        ...
        if(!Array.isArray(values)||values.length>60||values.some(item=>!isObject(item)||typeof item.exercise!=='string'||!item.exercise.trim()||!isFiniteValue(item.loadKg)||Number(item.loadKg)<=0||Number(item.loadKg)>700||!Number.isInteger(Number(item.reps))||Number(item.reps)<1||Number(item.reps)>10||(owns(item,'rpe')&&(!isFiniteValue(item.rpe)||Number(item.rpe)<6||Number(item.rpe)>10||!Number.isInteger(Number(item.rpe)*2)))||(owns(item,'setNumber')&&(!Number.isInteger(Number(item.setNumber))||Number(item.setNumber)<1||Number(item.setNumber)>20))||(owns(item,'bodyweightKg')&&(!isFiniteValue(item.bodyweightKg)||Number(item.bodyweightKg)<20||Number(item.bodyweightKg)>300))||(owns(item,'e1rmConfirmed')&&typeof item.e1rmConfirmed!=='boolean')))invalid('INVALID_OUTCOME','Le serie principali registrate per volume ed e1RM non sono valide.');
      }
      if(owns(session.outcome,'strengthPerformanceVersion')&&session.outcome.strengthPerformanceVersion!==2)invalid('INVALID_OUTCOME','La versione della registrazione del volume di forza non è valida.');
      ...
      if(owns(session.outcome,'deviceEvidence')){
        ...
        if(owns(evidence,'observations')){
          const observations=evidence.observations,numericFields=['durationMin','distanceKm','averagePaceSecPerKm','averageHr','maxHr','elevationGainM','calories','averageCadence','averageWatts','weightedWatts','relativeEffort','sportId','strain','kilojoule','percentRecorded','altitudeGainM','altitudeChangeM'];
          if(!isObject(observations)||!['strava','whoop'].some(source=>isObject(observations[source])))invalid('INVALID_OUTCOME','Le osservazioni collegate ai dispositivi non sono valide.');
          ['strava','whoop'].forEach(source=>{const item=observations[source];if(item===null||item===undefined)return;if(!isObject(item))invalid('INVALID_OUTCOME','Una fonte delle osservazioni dispositivo non è valida.');numericFields.forEach(field=>{if(owns(item,field)&&item[field]!==null&&!isFiniteValue(item[field]))invalid('INVALID_OUTCOME',`La metrica ${field} collegata al dispositivo non è valida.`);});[['hrZonePct',5],['hrZoneDurationsMin',6]].forEach(([field,length])=>{if(owns(item,field)&&item[field]!==null&&(!Array.isArray(item[field])||item[field].length!==length||item[field].some(value=>value!==null&&!isFiniteValue(value))))invalid('INVALID_OUTCOME',`Le ${field} collegate al dispositivo non sono valide.`);});['sport','scoreState'].forEach(field=>{if(owns(item,field)&&item[field]!==null&&typeof item[field]!=='string')invalid('INVALID_OUTCOME',`Il campo ${field} collegato al dispositivo non è valido.`);});});
        }
      }
    }
    return session;
  ...
  function validateWhoopRecords(name,value) {
    ...
    value.forEach(record=>{
      ...
      ['recoveryScore','restingHr','hrvMs','skinTempC','spo2Pct','dayStrain','energyKcal','maxHr','averageHr','sleepPerformancePct','respiratoryRate','sleepDurationMin','timeInBedMin','lightSleepMin','deepSleepMin','remSleepMin','awakeMin','sleepNeedMin','sleepDebtMin','sleepEfficiencyPct','sleepConsistencyPct','durationMin','strain','calories','kilojoule','percentRecorded','distanceKm','averagePaceSecPerKm','altitudeGainM','altitudeChangeM','sportId'].forEach(field=>{if(owns(record,field)&&record[field]!==null&&!isFiniteValue(record[field]))invalid('INVALID_WHOOP_DATA',`Il campo ${field} di una registrazione WHOOP non è valido.`);});
      if(scope==='sleep'&&typeof record.nap!=='boolean')invalid('INVALID_WHOOP_DATA','Il tipo di sonno WHOOP non è valido.');
      if(scope==='workout'){
        if(typeof record.name!=='string'||!record.name.trim()||typeof record.category!=='string'||!record.category.trim()||typeof record.gpsEnabled!=='boolean'||!Array.isArray(record.hrZonePct)||record.hrZonePct.length!==5||record.hrZonePct.some(item=>item!==null&&!isFiniteValue(item))||(owns(record,'hrZoneDurationsMin')&&(!Array.isArray(record.hrZoneDurationsMin)||record.hrZoneDurationsMin.length!==6||record.hrZoneDurationsMin.some(item=>item!==null&&!isFiniteValue(item))))||(owns(record,'scoreState')&&record.scoreState!==null&&typeof record.scoreState!=='string'))invalid('INVALID_WHOOP_DATA','Un allenamento WHOOP non è valido.');
      }
