(function () {
  ...
  function render() {
    ...
    ordered.forEach(session => {
      ...
      const text = document.createElement('div'); const title = document.createElement('h3'); title.textContent = session.title;if(session.demoDataset){const demo=document.createElement('span');demo.className='demo-badge';demo.textContent='DEMO';title.append(demo);}else if(session.planImport){const source=document.createElement('span');source.className='demo-badge';source.textContent='STORICO';title.append(source);}else if(session.baselinePlan){const base=document.createElement('span');base.className='demo-badge baseline-badge';base.textContent=session.coachPlan?'COACH':'BASE';base.title=session.coachPlan?`Piano Coach · revisione ${session.coachPlan.revision}`:'Traiettoria iniziale: verrà adattata dal Coach nella settimana operativa.';title.append(base);}if(session.adaptiveAdjustment){const adjusted=document.createElement('span');adjusted.className=`adaptive-badge ${paused?'paused':'active'}`;adjusted.textContent=paused?'SOSPESA':'ADATTATA';title.append(adjusted);}
  ...
  function updateStrengthPerformanceRow(row,formula){
    const sets=[...row.querySelectorAll('.strength-performance-set')],atLimit=sets.length>=20;sets.forEach((set,index)=>{set.querySelector('.strength-performance-set-index').textContent=`SERIE ${index+1}`;const remove=set.querySelector('.strength-performance-set-remove'),copy=set.querySelector('.strength-performance-set-copy');remove.hidden=sets.length===1;remove.disabled=sets.length===1;if(copy){copy.disabled=atLimit||copy.dataset.complete!=='true';copy.setAttribute('aria-label',`Copia serie ${index+1} ${row.dataset.exercise}`);}});const add=row.querySelector('.strength-performance-add-set');if(add){add.disabled=atLimit;add.textContent=atLimit?'Limite di 20 serie':'+ Aggiungi serie';}
  ...
  function strengthPerformanceSet(row,lift,saved,formula){
    ...
    const marker=document.createElement('div');marker.className='strength-performance-set-marker';const index=document.createElement('small');index.className='strength-performance-set-index';const actions=document.createElement('div');actions.className='strength-performance-set-actions';const copy=document.createElement('button');copy.type='button';copy.className='ghost small strength-performance-set-copy';copy.textContent='Copia';copy.title='Duplica questa serie sotto';const remove=document.createElement('button');remove.type='button';remove.className='row-action remove strength-performance-set-remove';remove.textContent='×';remove.setAttribute('aria-label',`Rimuovi serie ${lift.label}`);remove.addEventListener('click',()=>{set.remove();updateStrengthPerformanceRow(row,formula);});actions.append(copy,remove);marker.append(index,actions);
    ...
    const update=()=>{const hasAny=loadInput.value!==''||repsMetric.select.value!==''||rpeMetric.select.value!=='';loadInput.required=hasAny;repsMetric.select.required=hasAny;rpeMetric.select.required=hasAny;copy.dataset.complete=String(Boolean(loadInput.value&&repsMetric.select.value&&rpeMetric.select.value));const value=strengthModel.estimateE1rm(loadInput.value,repsMetric.select.value,{externalLoad:lift.externalLoad,bodyweightKg:athleteWeightKg(),formula,rpe:rpeMetric.select.value}),rir=strengthModel.rirFromRpe(rpeMetric.select.value);previewLabel.textContent=`e1RM${rir===null?'':' · RIR '+rir.toLocaleString('it-IT')}`;previewValue.textContent=value===null?'—':`${lift.externalLoad?'+':''}${value.toLocaleString('it-IT',{maximumFractionDigits:1})} kg`;updateStrengthPerformanceRow(row,formula);};
    copy.addEventListener('click',()=>{const duplicate=strengthPerformanceSet(row,lift,strengthModel.copySetValues({loadKg:loadInput.value,reps:repsMetric.select.value,rpe:rpeMetric.select.value}),formula);set.after(duplicate);updateStrengthPerformanceRow(row,formula);duplicate.querySelector('.strength-performance-set-copy')?.focus();});
  ...
  function toggleOutcomeFields() {
    ...
    const hasDistance=['running','swimming','cycling'].includes(outcomeForm.dataset.category);distanceField.hidden=!hasDistance;distanceInput.disabled=skipped||!hasDistance;
  ...
  function formatZones(values,suffix=' min'){
    if(!Array.isArray(values))return null;const visible=values.map((value,index)=>Number.isFinite(Number(value))&&Number(value)>0?`Z${index} ${Number(value).toLocaleString('it-IT',{maximumFractionDigits:1})}${suffix}`:null).filter(Boolean);return visible.length?visible.join(' · '):null;
  }
  function renderObservedEvidence(evidence,hasManualOutcome=false){
    ...
    add('DISTANZA STRAVA',evidence.observed.stravaDistanceKm===null?null:`${evidence.observed.stravaDistanceKm} km`,'GPS Strava');add('PASSO STRAVA',formatPace(evidence.observed.stravaAveragePaceSecPerKm),'tempo in movimento');
    add('DISTANZA WHOOP',evidence.observed.whoopDistanceKm===null?null:`${evidence.observed.whoopDistanceKm} km`,'GPS WHOOP');add('PASSO WHOOP',formatPace(evidence.observed.whoopAveragePaceSecPerKm),'durata intera della sessione');
    ...
    add('POTENZA STRAVA',evidence.observed.averageWatts===null?null:`${Math.round(evidence.observed.averageWatts)} W`,evidence.observed.weightedWatts===null?'media':`ponderata ${Math.round(evidence.observed.weightedWatts)} W`);add('DISLIVELLO STRAVA',evidence.observed.stravaElevationGainM===null?null:`${Math.round(evidence.observed.stravaElevationGainM)} m`,'positivo');add('SFORZO STRAVA',evidence.observed.relativeEffort,'sforzo relativo');
    add('WHOOP STRAIN',evidence.observed.whoopStrain,'osservato');add('ENERGIA WHOOP',evidence.observed.whoopCalories===null?null:`${Math.round(evidence.observed.whoopCalories)} kcal`,evidence.observed.whoopKilojoule===null?'':`${Math.round(evidence.observed.whoopKilojoule)} kJ`);add('DISLIVELLO WHOOP',evidence.observed.whoopAltitudeGainM===null?null:`${Math.round(evidence.observed.whoopAltitudeGainM)} m`,'positivo');add('COPERTURA WHOOP',evidence.observed.whoopPercentRecorded===null?null:`${Math.round(evidence.observed.whoopPercentRecorded)}%`,'tempo registrato');add('ZONE FC WHOOP',formatZones(evidence.observed.whoopHrZoneDurationsMin),'Z0–Z5 · minuti');
    const note=document.getElementById('outcome-observed-note');if(evidence.warnings.length){note.className='warning';note.textContent=evidence.warnings.join(' ');}else{note.className='';note.textContent=hasManualOutcome?'La durata reale usa WHOOP quando disponibile; RPE, dolore, note e dettagli restano quelli registrati manualmente.':'La durata usa WHOOP quando disponibile; la distanza preferisce Strava e usa WHOOP come fallback. RPE, confronto col previsto e dolore richiedono sempre la tua valutazione.';}
  }
  ...
  function renderOutcomeRecord(session){
    const holder=document.getElementById('outcome-record-view'),outcome=session.outcome,evidence=currentEvidenceIndex.get(session.id)||null;holder.replaceChildren();if(!outcome)return;
    ...
    if(outcome.status==='skipped')addMetric('MOTIVO',outcome.skipReason?skipReasonModel.label(outcome.skipReason):'Non specificato');
      ...
      if(['running','swimming','cycling'].includes(session.category))addMetric('DISTANZA REALE',metricValue(outcome.actualDistanceKm,' km'));
    ...
    const evidenceLabel=evidenceSourceLabel(outcome.deviceEvidence);if(evidenceLabel)addMetric('DATI COLLEGATI',evidenceLabel);const observed=evidence?.observed||{},stored=outcome.deviceEvidence?.observations||{},strava=stored.strava||(evidence?.stravaActivityId?{distanceKm:observed.stravaDistanceKm,averagePaceSecPerKm:observed.stravaAveragePaceSecPerKm,averageHr:observed.stravaAverageHr,maxHr:observed.stravaMaxHr}:null),whoop=stored.whoop||(evidence?.whoopWorkoutId?{distanceKm:observed.whoopDistanceKm,averagePaceSecPerKm:observed.whoopAveragePaceSecPerKm,averageHr:observed.whoopAverageHr,maxHr:observed.whoopMaxHr,strain:observed.whoopStrain,percentRecorded:observed.whoopPercentRecorded}:null);
    if(strava?.distanceKm)addMetric('STRAVA · DISTANZA',`${Number(strava.distanceKm).toLocaleString('it-IT',{maximumFractionDigits:2})} km`);if(strava?.averagePaceSecPerKm)addMetric('STRAVA · PASSO',formatPace(strava.averagePaceSecPerKm));if(strava?.averageHr)addMetric('STRAVA · FC',`${Math.round(strava.averageHr)} bpm · max ${strava.maxHr?Math.round(strava.maxHr):'—'}`);
    if(whoop?.distanceKm)addMetric('WHOOP · DISTANZA',`${Number(whoop.distanceKm).toLocaleString('it-IT',{maximumFractionDigits:2})} km`);if(whoop?.averagePaceSecPerKm)addMetric('WHOOP · PASSO',formatPace(whoop.averagePaceSecPerKm));if(whoop?.averageHr)addMetric('WHOOP · FC',`${Math.round(whoop.averageHr)} bpm · max ${whoop.maxHr?Math.round(whoop.maxHr):'—'}`);if(whoop?.strain)addMetric('WHOOP · STRAIN',Number(whoop.strain).toLocaleString('it-IT',{maximumFractionDigits:1}));if(whoop?.percentRecorded)addMetric('WHOOP · COPERTURA',`${Math.round(whoop.percentRecorded)}%`);holder.append(metrics);
  ...
  outcomeForm.addEventListener('submit',event=>{
    ...
    const actualDistanceKm=!skipped&&['running','swimming','cycling'].includes(existing.category)&&data.get('actualDistanceKm')?Number(data.get('actualDistanceKm')):null;let deviceEvidence=null;
  ...
  function replaceBaselinePlan(newSessions,options={}){const fromDate=options.fromDate||localDate(),preserved=sessions.filter(item=>item.coachPlan||!item.baselinePlan||item.outcome||item.manualOverride||item.date<fromDate),occupied=new Set(preserved.filter(item=>item.date>=fromDate&&(!item.baselinePlan||item.coachPlan)).map(item=>item.date)),incoming=structuredClone(Array.isArray(newSessions)?newSessions:[]).filter(item=>!occupied.has(item.date));sessions=[...preserved,...incoming];selectedIds=selectionModel.prune(selectedIds,sessions);save();render();document.dispatchEvent(new CustomEvent('rc:sessions-updated',{detail:{reason:'baseline-plan-replaced',fromDate,signature:options.signature||null}}));}
