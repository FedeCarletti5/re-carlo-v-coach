(function(){
  ...
  function renderAdaptive(model){
    if(!adaptivePanel)return;const coach=model.adaptiveCoach;adaptivePanel.hidden=coach.visible===false;adaptivePanel.replaceChildren();if(adaptivePanel.hidden)return;adaptivePanel.className=`panel today-adaptive-panel ${coach.level}${adaptiveCollapsed?' is-collapsed':''}`;
    const head=element('div','panel-head');const copy=element('div');copy.append(element('small','',coach.kicker),element('h2','',coach.title));const status=element('span',`today-adaptive-status ${coach.tone}`,coach.statusLabel);const headActions=element('div','panel-head-actions');const toggle=element('button','panel-collapse-toggle');toggle.type='button';toggle.setAttribute('aria-controls','today-adaptive-body');const toggleLabel=element('span','',adaptiveCollapsed?'Apri':'Nascondi');const chevron=element('span','panel-collapse-chevron');chevron.setAttribute('aria-hidden','true');toggle.append(toggleLabel,chevron);const syncToggle=()=>{adaptivePanel.classList.toggle('is-collapsed',adaptiveCollapsed);toggle.setAttribute('aria-expanded',String(!adaptiveCollapsed));toggle.setAttribute('aria-label',adaptiveCollapsed?'Apri adattamento':'Nascondi adattamento');toggleLabel.textContent=adaptiveCollapsed?'Apri':'Nascondi';};toggle.addEventListener('click',()=>{adaptiveCollapsed=!adaptiveCollapsed;syncToggle();});syncToggle();headActions.append(status,toggle);head.append(copy,headActions);
    ...
    const actions=element('div','today-adaptive-actions');if(coach.applied){const done=element('button','primary today-adaptive-done','✓ Adattamento applicato');done.type='button';done.disabled=true;const plan=element('button','ghost','Apri il piano');plan.type='button';plan.addEventListener('click',()=>window.rcNavigation?.show('plan'));actions.append(done,plan);}else{const preview=element('button','primary',coach.reviewRequired?'Rivedi adattamento':coach.stale?'Apri adattamento':'Valuta adattamento');preview.type='button';preview.addEventListener('click',()=>{window.rcNavigation?.show('plan');setTimeout(()=>window.rcGenerator?.open?.(coach.reviewRequired?{weekStart:model.weekStart,mode:'review'}:{}),0);});actions.append(preview);}const recap=element('button','ghost','Apri il recap completo');recap.type='button';recap.addEventListener('click',()=>window.rcNavigation?.show('recap'));actions.append(recap);
  ...
  function renderNoSession(model){
    ...
    const manual=element('button','ghost','Incolla un allenamento');manual.type='button';manual.addEventListener('click',()=>window.rcSessions.openEditor());
    const actions=element('div','today-session-actions');actions.append(action,manual);sessionPanel.append(head,empty,actions);
  }
  ...
  function renderWeek(model){
    ...
    else if(!model.week.completedCount)noteText=model.week.sessions?'Le sedute programmate appariranno qui dopo la registrazione.':'Importa un piano o aggiungi una seduta per iniziare a costruire lo storico.';
