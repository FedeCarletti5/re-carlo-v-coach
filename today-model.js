(function (root, factory) {
  ...
  function buildAdaptiveCoach(analysis={},application={applied:false,stale:false,application:null}){
    ...
    const decisionTitle=analysis?.label||meta.title;if(application.applied){return{level:'applied',tone:'good',title:'Adattamento applicato',decisionTitle,kicker:'ADATTAMENTO · APPLICATO',statusLabel:'✓ APPLICATO',summary:`La proposta “${decisionTitle}” è già stata applicata alla settimana corrente.`,confidence,whoop,impact:{label:'Applicato alla settimana',detail:impact.label==='Piano invariato'?'La struttura è stata confermata senza correzioni.':impact.detail},reasons,applied:true,stale:false,visible:false,appliedAt:application.application?.appliedAt||null};}
    const reviewSummary=reviewRequired?`Hai registrato “${reviewTrigger?.title||'una seduta chiave'}”. L’app ha ricalcolato i segnali e deve confrontare la decisione con ${application.remainingCount||0} sedut${application.remainingCount===1?'a':'e'} ancora apert${application.remainingCount===1?'a':'e'} prima di confermare il resto della settimana.`:null;
    const changed=(Array.isArray(application.staleReasons)?application.staleReasons:[application.staleReason]).filter(item=>item&&item!=='key-outcome').map(item=>staleReasonLabels[item]||staleReasonLabels['analysis-changed']);const unique=[...new Set(changed)],changedLabel=unique.length?`Il Coach ha rilevato un cambiamento in: ${unique.join(', ')}`:'I fattori della decisione sono cambiati';
    const meaningful=reviewRequired||Boolean(application.stale)||['protect','reduce'].includes(level);
    return{level,tone:reviewRequired?'warn':meta.tone,title:reviewRequired?'Aggiornamento della settimana':decisionTitle,decisionTitle,kicker:reviewRequired?'ADATTAMENTO · DA RIVEDERE':application.stale?'ADATTAMENTO · DA RIVEDERE':'ADATTAMENTO DISPONIBILE',statusLabel:reviewRequired?'SEDUTA CHIAVE REGISTRATA':`AFFIDABILITÀ ${confidence.toUpperCase()}`,summary:reviewSummary||(application.stale?`${analysis?.summary||'La decisione corrente è cambiata.'} ${changedLabel} dopo l’ultima conferma: rivedi l’anteprima prima di aggiornare di nuovo la settimana.`:analysis?.summary||'Dati recenti ancora limitati: l’app mantiene una proposta prudente e modificabile.'),confidence,whoop,impact,reasons,applied:false,stale:Boolean(application.stale),visible:meaningful,reviewRequired,reviewTrigger,appliedAt:null};
  }
  ...
  function issueRegion(issue){const zone=String(issue?.zone||''),label=String(issue?.zoneLabel||'');if(/^(hip|adductor|quad|knee|shin|ankle|foot|glute|hamstring|calf)(-|$)/.test(zone)||/(anca|adduttor|quadricipite|ginocchio|tibia|caviglia|piede|gluteo|femorale|polpaccio)/i.test(label))return 'lower';if(/^(shoulder|upper-arm|triceps|elbow|forearm|wrist)(-|$)/.test(zone)||/(spalla|braccio|tricipite|gomito|avambraccio|polso)/i.test(label))return 'upper';if(/^(chest|abdomen|upper-back|lower-back|neck)(-|$)/.test(zone)||/(petto|addome|dorso|schiena|lombare|collo)/i.test(label))return 'trunk';if(/^head(?:-|$)/.test(zone)||/testa/i.test(label))return 'head';return 'unknown';}
  ...
  function segmentTargetLabel(value){
  ...
  function prescriptionFor(session) {
    ...
    if (session.category==='running'&&Array.isArray(details.runBlocks)&&details.runBlocks.length) {
      ...
      return details.runBlocks.map(item=>{
        if(item.type==='repeat')return {label:`${number(item.repeats)||1}× sequenza`,value:(item.steps||[]).map(formatSegment).join(' / ')||'Fasi da definire',phase:'repeat',steps:(item.steps||[]).map(step=>({label:phaseLabels[step.phase]||'Fase',phase:step.phase||'free',intensity:step.intensity||item.intensity||'tempo',...segmentMetrics(step)})),intensity:item.intensity||item.steps?.[0]?.intensity||'tempo'};
        return {label:phaseLabels[item.phase]||'Blocco',value:formatSegment(item),phase:item.phase||'free',...segmentMetrics(item),intensity:item.intensity||'easy'};
      });
    }
    ...
    if(session.category==='cycling'&&Array.isArray(details.rideBlocks)&&details.rideBlocks.length){
      ...
      const blocks=details.rideBlocks.map(item=>item.type==='repeat'
        ?{label:`${number(item.repeats)||1}× sequenza`,value:(item.steps||[]).map(formatSegment).join(' / ')||'Fasi da definire',phase:'repeat',steps:(item.steps||[]).map(step=>({label:phaseLabels[step.phase]||'Fase',phase:step.phase||'free',intensity:step.intensity||item.intensity||'tempo',...segmentMetrics(step)})),intensity:item.intensity||item.steps?.[0]?.intensity||'tempo'}
        :{label:phaseLabels[item.phase]||'Blocco',value:formatSegment(item),phase:item.phase||'free',...segmentMetrics(item),intensity:item.intensity||'easy'});
